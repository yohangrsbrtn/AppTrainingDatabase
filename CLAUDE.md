...
- **`repas`** : `{ id, template_id, nom, ordre, variante_index INT DEFAULT 0, commentaire }` — repas avec même `(template_id, ordre)` mais `variante_index` différent = repas équivalents (swipeables côté client). FK column = **`template_id`** (pas `diete_template_id`). `commentaire` (TEXT, `sql/2026-08-09_commentaire_repas.sql`) : note du coach visible par le client (ex: "ajouter de la levure"), éditable dans l'éditeur diète console, affichée sous le nom du repas dans `diete.js`. `diete_templates.description` (déjà existante) sert de commentaire au niveau de la diète entière, affiché en haut de "Ma diète" — champ resté purement interne au coach jusqu'ici, désormais visible côté client.
